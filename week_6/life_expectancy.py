#made a loop to keep requesting more information until quit. Have the min/max/average for a country, and the min/max/average for a year. 
with open('week_6/life_expectancy.csv') as life_exectancy_file:
    life_expectancy_data = life_exectancy_file.readlines()[1:]
    ...
    while user_action != len(choices) +1 :    
        ...
        if user_action == 7:
            ...
            if any(user_choice_country_abbreviation == code for code, _ in country_code_list): 
                ...
                print(f"Average life span: {country_average:.2f}. Worst on record: {record_low:.2f} years in {record_low_year}. Best on record: {record_high:.2f} in {record_high_year}.")
